...
You have ${numQuestions} questions to create. You MUST assign each question to a DIFFERENT source when possible, but cycle through sources if needed:

${generateSourceAssignmentInstructions(numQuestions)}

IMPORTANT: 
- Try to use different sources for consecutive questions
- If you have fewer sources than questions, cycle through them intelligently
- Ensure maximum variety by spacing out repeated sources
- Never use the same source for more than 2 consecutive questions
...
export function createDiversityFirstPromptWithSourceCount(
  config: DiversityPromptConfig,
  context: string,
  questionProfiles: QuestionDiversityProfile[],
...
IMPORTANT: The material below comes from ${actualSourceCount} DIFFERENT SOURCES. You MUST use different sources for different questions to ensure maximum diversity.

COURSE MATERIAL:
${context}

SOURCE IDENTIFICATION:
Look for these exact markers in the material above:
- "--- SOURCE 1:" marks the first source
- "--- SOURCE 2:" marks the second source  
- "--- SOURCE 3:" marks the third source
- And so on...

You MUST reference content from the specific source assigned to each question.

ASSIGNMENT INSTRUCTIONS:
You have ${numQuestions} questions to create using ${actualSourceCount} available sources:

${generateSourceAssignmentInstructionsWithCount(numQuestions, actualSourceCount)}
...
function generateSourceAssignmentInstructions(numQuestions: number): string {
...
export function createAggressiveDiversityPrompt(
  config: DiversityPromptConfig,
  context: string,
  actualSourceCount: number
): string {
...
COURSE MATERIAL WITH SOURCE MARKERS:
${context}
...
2. DO NOT use the same source for multiple questions
3. DO NOT mix content from different sources in one question
4. The sourceUsed field MUST match the assigned source
5. If you cannot find content in the assigned source, create a question about a related topic from that source
...
export function createRandomIndexDiversityPrompt(
  config: DiversityPromptConfig,
  context: string,
...
RANDOMLY SELECTED COURSE MATERIAL:
${context}
